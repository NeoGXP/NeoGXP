...
namespace GXPEngine.Core
{
    public class SoloudSoundSystem : SoundSystem
    {
        ...
        public override SoundChannel PlaySound(IntPtr id, bool paused)
        {
            if (id == IntPtr.Zero) return new SoundChannel(0);
            return new SoundChannel(Soloud.Soloud_playEx(_device, id, 1.0f, 0.0f, paused, 0));
        }

		public override SoundChannel PlaySound(IntPtr id, bool paused, float volume, float pan)
		{
			if (id == IntPtr.Zero) return new SoundChannel(0);
			return new SoundChannel(Soloud.Soloud_playEx(_device, id, volume, pan, paused, 0));
		}
